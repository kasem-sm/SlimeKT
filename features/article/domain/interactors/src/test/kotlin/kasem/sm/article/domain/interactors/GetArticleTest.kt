...
import kasem.sm.article.domain.interactors.utils.defaultPair
import kasem.sm.article.domain.interactors.utils.getMockDto
...
import kasem.sm.article.domain.interactors.utils.mockSuccessResponseWithNullData
...
class GetArticleTest {
    ...
    fun assertApiCallErrorIsAnExceptionStage_and_InsertNotCalled() = runBlocking {
        coEvery { apiMock.getArticleById(1) } returns Result.failure(Exception())

        val stage = useCase.execute(1).first()

        (stage as Stage.Exception).throwable shouldBe Exception()
